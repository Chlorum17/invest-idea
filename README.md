...
1. [Важные нюансы](#важные-нюансы);
2. [Запуск проекта](#запуск-проекта);
3. [Информация для тестирования](#информация-для-тестирования);

   3.1. [Содержимое конфигурационного файла](#содержимое-конфигурационного-файла);

   3.2. [Данные для аутентификации](#данные-для-аутентификации);

   3.3. [Рейтинг идеи](#рейтинг-идеи);

   3.4. [График доходности](#график-доходности);
...
4. [Описание эндпоинтов](#описание-эндпоинтов);
   ...
   4.3. [Invest Idea](#invest-idea);

   4.4. [Idea Rating](#idea-rating).
...
3.  Логика графика доходности, отличается от того, что на сайте.
...
1. Склонировать репозиторий командой git clone https://github.com/Chlorum17/invest-idea.git
2. Установить зависимости командой npm ci --production
3. Создать в корне проекта .env файл со [следующим содержимым](#содержимое-конфигурационного-файла)
...
Нужно создать в корне проекта .env файл, и записать в него следующее:
...
Создано 5 юзеров, от **test1@gmail.com** до **test5@gmail.com** соответсвенно, пароль одинаковый.
...
Чтобы иметь возможность проголосовать - нужно аутентифицироваться, авторизация осуществляется за счет access токена.

### График доходности

Данные для получения графика доходности добавлены в идею со следующим id - **60a4f437e87fbb47482f28eb**
Эндпоинт для добавления данных, если потребуется [PATCH http://localhost:8080/api/invest-ideas/update/:ideaId](#PATCH-http://localhost:8080/api/invest-ideas/update/:ideaId)

#### Тело запроса эндпоинта для добавления данных
...
Позволяет получить массив записей значений дохода, записи представлены в виде объектов, где поля:

- **value** - значение дохода;
- **date** - дата фиксации значения дохода
...
#### PATCH http://localhost:8080/api/invest-ideas/update:ideaId

Эндпоинт для добавления записей в поле currentIncome, сущности Invest Idea, для функционала графиков.
